...
**Revised:** 2026-09-25 — **action set replaced.** The module now ships exactly five actions modelled on the Cortex-Analyzers VMRay analyzer: `GetSamplesByHash` (batch lookup), `SubmitUrlSample` (submit + wait for every submission), `BuildReport` (the analyzer's `_build_report`: per-sample analyses of the latest submission, VTIs, MITRE, IOCs, classifications, threat names, child samples to `max_recursion_depth`), `RenderReport`, `ReportToIndicators`. Everything else — `SearchSample`, `SubmitUrl`/`SubmitFile`/`SubmitAndWait`/`SubmitAndEnrich`, `GetAnalysisDetails`, `RenderSummary`, `IocsToIndicators`, `GetSample`/`GetReportPdf`/`GetScreenshots` — and their three playbooks were removed; the action tables below are historical. Current playbook: `VMRay_Manual_Report` (manual trigger only).
...
- **Template B — Automatic** *(revised 2026-09-24 — hash **and** URL)*: Alert Created → urgency ≥ threshold → Get Alert → Get Events → three `Read JSON File` extractions → hash found? → `SearchSample` → `GetAnalysisDetails`; no hash, or hash unknown to VMRay → http(s) URL found? → `SubmitAndEnrich` (URL detonation). Both branches converge on one `RenderSummary` → comment → malicious? → `IocsToIndicators` → IOC collection. 24 nodes (23 for the manual copy).
  ...
  - **Branches do NOT converge — confirmed on a live tenant (2026-09-24).** A shared downstream chain reading `node.9 if (node.9 is defined and node.9) else node.11['analysis']` fails with **"Invalid Node Run Reference"**: Sekoia resolves every `node.N` reference itself, before Jinja evaluates anything, and errors if that node never ran on the current path — so `is defined` guards can't help. Rule: a node may only reference nodes that run on **every** path into it. Each branch now carries its own render → comment → verdict → IOC chain (19–23 duplicate 12–16), 24/23 nodes. (Resolves the spike-0.6 question below about referencing a never-executed node — it's an error, not falsy.)
